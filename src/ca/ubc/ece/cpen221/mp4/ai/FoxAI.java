...
public class FoxAI extends AbstractAI {
	...
	public Command getNextAction(ArenaWorld world, ArenaAnimal animal) {
		...
		int eatDesire = (int) (closest * (1.0 - ((double) animal.getEnergy() / animal.getMaxEnergy())));
		for(Item i : world.searchSurroundings(animal)){
			if(i.getStrength() > animal.getStrength()){
				N = N - (closest * getYDist(animal, i));
				S = S + (closest * getYDist(animal, i));
				E = E - (closest * getXDist(animal, i));
				W = W + (closest * getXDist(animal, i));
			}
			else if (i.getMeatCalories() > 0){
		...
		for(int i = 3; i >= 0; i--){
			...
			if(N == weights[i]){
				...
				else if(activeItem.getMeatCalories() > 0)
